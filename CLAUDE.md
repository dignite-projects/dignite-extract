...
3. **文本提取能力栈（三层契约 + 多 Provider）**——核心可插拔点
   ...
   - **OCR Provider 实现**：`Dignite.DocumentAI.Ocr.VisionLlm`（Host 当前默认，#259：多模态视觉 LLM 转写照片 / 热敏票据 / 图片型 PDF，复用 host 注册的 keyed vision `IChatClient`，PDF 经 PDFium + SkiaSharp 逐页光栅化；仅依赖 `Dignite.DocumentAI.Ocr` 契约层）、`Dignite.DocumentAI.Ocr.PaddleOcr`（本地 sidecar，PP-StructureV3 走 CPU 即可，输出 Markdown）与 `Dignite.DocumentAI.Ocr.AzureDocumentIntelligence`（云方案，高精度）；`IOcrProvider` 互斥——Host 三选一启用，切换时 `[DependsOn]` + `.csproj ProjectReference` 两处同步（切换涉及 VisionLlm 时还需同步 host `ConfigureAI` 的 keyed vision `IChatClient` 注册）
...
4. **Dignite.DocumentAI.Installer（ABP Studio 安装打包项目）**——把 `.abpmdl` / `.abppkg` 模块描述文件打进 NuGet content 供 ABP Studio 安装模块用；仅依赖 `Volo.Abp.VirtualFileSystem`，**不参与运行时依赖拓扑**
...
- 在 `ConfigureServices()` 中：
  - 配置 OCR Provider（默认 VisionLlm，可切 PaddleOCR / Azure Document Intelligence）+ Markdown Provider（ElBruno MarkItDown）
  - 注册三个 keyed `IChatClient`（`TitleGenerator` / `Structured` / `Vision`，`ConfigureAI` 默认绑 OpenAI 兼容端点，其他 wire protocol 由 host 替换该方法，按 `Microsoft.Extensions.AI` 生态选择）—— 分别供标题生成、结构化输出路径（LLM 分类 / 类型绑定字段抽取（B 机制）/ 选柜 / slug / 字段定义草拟）、VisionLlm OCR 使用。**不注册 `IEmbeddingGenerator`**——通道层不做向量化（见 OUT of scope）
...
```
Host Application
    ├── 注册三个 keyed IChatClient（TitleGenerator / Structured / Vision；不注册 IEmbeddingGenerator）
    └── DependsOn:
        ...
        └── Dignite.DocumentAI.Ocr.VisionLlm（OCR Provider，当前默认，#259；可切换 Ocr.PaddleOcr / Ocr.AzureDocumentIntelligence）
...
Dignite.DocumentAI.Ocr（OCR Provider 最小契约层，无其他 Document AI 项目依赖）
    ...
    └── 被 Ocr.VisionLlm / Ocr.PaddleOcr / Ocr.AzureDocumentIntelligence 等 Provider 实现项目引用
```
...
| `DocumentTypeId` | 分类 pipeline | 分类结果（#207：内部按不可变 `DocumentType.Id` 关联；外部 wire-format——REST / MCP / ETO——仍输出 `DocumentTypeCode` 字符串，由读路径 join `DocumentType` 解析，TypeCode rename 不级联本表） |
| `ClassificationConfidence` / `ReviewDisposition` / `ReviewReasons` / `RejectionReason` | 分类 pipeline + 人工审核 | #284 双轴审核模型：`ReviewDisposition` 是操作员处置轴（NotReviewed / Confirmed / Rejected），`ReviewReasons` 是待审原因 flags 轴（每个 bit 由唯一一个 pipeline 阶段维护）；`RejectionReason` 仅 Rejected 时有值（操作员必填的拒绝说明） |
| `LifecycleStatus` | 流水线编排 | 宏观生命周期状态 |
| `Language` | OCR / 抽取阶段 | ISO 639-1 / IETF tag |

`FileOrigin`（Owned Entity）含 `BlobName`（BlobStore Key）/ `OriginalFileName` / `FileSize` / `ContentType` / `ContentHash` 等上传时元数据。无独立"Filename / Size / Format"系统字段——读路径直接 `d.FileOrigin.OriginalFileName` 等。另有 `CabinetId`（可空 Guid，#194）——文件柜人工组织归属维度，由操作员设定，**正交于 pipeline**（OCR / 分类 / 字段抽取不读不写），不属于 pipeline 自动产生的系统字段。
...
1. **文本类型字段：永远只有 `Markdown` 一个。** 这是 Markdown-first 在持久化层的硬约束（已被 `Document.SetMarkdown` 的 immutability 强保护在代码层面执行）。任何派生文本（Summary / Outline / SectionsJson）走 `MarkdownStripper.Strip` 或切块器在消费侧投影，**不持久化**。`Title` 是 Markdown 派生的展示快照（不可变），不是新文本载荷；`RejectionReason` 是操作员拒绝审核时的人工说明（#284，不是文档内容）
...
以下安全约定适用于 Document AI 内部 LLM 调用路径（内置 LLM 分类、统一字段抽取（B 机制，Host 字段 + 租户字段）、标题生成、选柜 / slug / 字段定义草拟建议、VisionLlm OCR 等）：
